...
describe('Course Integration', () => {
  ...
  describe('GET /courses/:id/studentTaskProgress/:userId/:assignmentName', () => {
    it('should return 401 if user is not authorized', async () => {
      await supertest()
        .get(`/courses/1/studentTaskProgress/1/assignment1`)
        .expect(401);
    });
    ...
    it('should return 404 if invalid course or invalid user', async () => {
      ...
      // invalid course
      const resp3 = await supertest({ userId: professor.id }).get(
        `/courses/27/studentTaskProgress/${student.id}/assignment1`,
      );
      expect(resp3.status).toBe(404);

      // invalid student
      const resp = await supertest({ userId: student.id }).get(
        `/courses/${course.id}/studentTaskProgress/30/assignment1`,
      );
      expect(resp.body).toEqual({});
    ...
    it('should return {} if no assignment is found (student has not started assignment)', async () => {
      ...
      const resp2 = await supertest({ userId: professor.id }).get(
        `/courses/${course.id}/studentTaskProgress/${student.id}/assignment2`,
      );
    ...
    it('should return the assignment progress for a students own progress', async () => {
      ...
      const resp = await supertest({ userId: student.id }).get(
        `/courses/${course.id}/studentTaskProgress/${student.id}/assignment1`,
      );
      expect(resp.body).toEqual({
        task1: {
          isDone: true,
