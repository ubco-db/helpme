...
/**
 * Used for the popular times graph in the course page.
 * Very little was changed during the great frontend refactor.
 */
const TimeGraph: React.FC<TimeGraphProps> = ({
  ...
  return width < 10 ? null : (
    ...
    <div className="time-graph-container relative">
      <svg ref={containerRef} width={width} height={height}>
        ...
        <Group left={LEFT_MARGIN} top={TOP_MARGIN}>
          {values.map((value, i) => {
            ...
            const barX = (xScale(i) ?? 0) + (barWidth * (1 + BAR_PADDING)) / 2
        ...
        <Group top={TOP_MARGIN} left={LEFT_MARGIN}>
          <AxisLeft
            ...
            hideTicks={true}
            tickValues={yAxisTickValues}
            tickFormat={(hour) => formatWaitTime(Number(hour))}
            tickLabelProps={() => ({
              fill: '',
              fontSize: 11,
              textAnchor: 'end',
