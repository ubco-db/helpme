...
Staff edits to question and quiz configuration write to the mutable `EmbeddableQuestionModel` / `EmbeddableQuizModel` rows. Those edits are not versioned and leave no history table. Revisit that only if the professor needs rollback of question configuration.
...
- **HelpMe score validation.** After the one chatbot call, HelpMe validates the returned grade against the course's score scale and caps. An invalid grade fails without saving. HelpMe's single-call retry budget deliberately avoids nesting retries inside the chatbot's structural repair loop.
