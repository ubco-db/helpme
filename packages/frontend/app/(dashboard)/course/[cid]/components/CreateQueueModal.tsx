...
export default function CreateQueueModal({
  ...
  role,
  lastName,
}: CreateQueueModalProps): ReactElement {
  ...
  const onIsOnlineUpdate = (isOnline: boolean) => {
  ...
  const handleSubmit = async () => {
    ...
    // validate the queue input and create the queue
    await form
      .validateFields()
      .then(async (queueRequest) => {
        await API.queues
          .createQueue(
            cid,
          ...
          .then(() => {
            mutateCourse()
            message.success(
              `Created a new queue ${queueRequest.officeHourName}`,
            )
            onSuccessfulSubmit()
            return
          })
